...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
cddm.randomWalk <- function(trials, mu1, mu2, boundary, tzero=0.1, drift.Coeff=1, dt=0.00015){
  sqDT <- sqrt(dt)
  s.init <- c(0,0) 
  ...
  for(a in 1:trials){   
    ...
    for(t in 2:iter){
      ...
      if(pass >= boundary){
        finalT[a] <- t+(tzero/dt)   #Total no. of iterations required on each trial
        break
      }
    }
    ...
    while(not.finished){
      ...
      for(t in 2:iter){
        ...
        if(pass >= boundary){
          ...
          finalT[a] <- (t+added_iterations)+(tzero/dt)   #Total no. of iterations required on each trial
          break
        }
      }
