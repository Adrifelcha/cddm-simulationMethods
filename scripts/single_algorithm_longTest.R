#########################################################################
# S I N G L E     A L G O R I T H M     L O N G     T E S T ##########
...
param_sets <- list(
    Fastest = list(par = list(
        mu1 = -2.5, mu2 = 2.5,    # Strong drift at 135°
        boundary = 5, tzero = 0.1
    )),
    Fast = list(par = list(
        mu1 = -2.0, mu2 = 2.0,    # Strong drift at 135°
        boundary = 5, tzero = 0.1
    )),
    Medium = list(par = list(
        mu1 = -1.5, mu2 = 1.5,    # Medium drift at 135°
        boundary = 5, tzero = 0.1
    )),
    Slow = list(par = list(
        mu1 = -1.0, mu2 = 1.0,    # Weak drift at 135°
        boundary = 5, tzero = 0.1
    )),
    Slowest = list(par = list(
        mu1 = 0.0, mu2 = 1.0,    # Weak upward drift
        boundary = 5, tzero = 0.1
    ))
)

nParamSets <- length(param_sets)
nTrials <- length(trial_sizes)

cat("Fastest drift length:", sqrt(sum(rep(param_sets$Fastest$par$mu1, 2)^2)), "\n")
cat("Fast drift length:", sqrt(sum(rep(param_sets$Fast$par$mu1, 2)^2)), "\n")
cat("Medium drift length:", sqrt(sum(rep(param_sets$Medium$par$mu1, 2)^2)), "\n")
cat("Slow drift length:", sqrt(sum(rep(param_sets$Slow$par$mu1, 2)^2)), "\n")
cat("Slowest drift length:", sqrt(sum(rep(param_sets$Slowest$par$mu1, 2)^2)), "\n")
cat("Boundary:", param_sets$Fastest$par$boundary, "\n")
cat("Tzero:", param_sets$Fastest$par$tzero, "\n")
cat("Theta:", atan2(param_sets$Fastest$par$mu2, param_sets$Fastest$par$mu1), "\n\n")
...
figname_results <- sprintf(here("results", "longTest_%s_%s.pdf"), method_tested, format(Sys.Date(), "%Y%m%d"))
plot_algorithm_performance(results, param_sets, trial_sizes, n_reps, method_tested, filename = figname_results)    


if(method_tested == "RandomWalk") {
    figname_circPrecision <- sprintf(here("results", "longTest_%s_%s_circPrecision.pdf"), method_tested, format(Sys.Date(), "%Y%m%d"))
...
# Usage example:
figure_cdf <- sprintf(here("results", "longTest_%s_%s_cdfs.pdf"), method_tested, format(Sys.Date(), "%Y%m%d"))
...
filename_prefix <- here("results", 
                       sprintf("longTest_%s_%s_metrics", 
                              method_tested, 
