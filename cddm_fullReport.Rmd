---
title: "Exploring different sampling algorithms for the CDDM"
...
The circular drift-diffusion model (CDDM) is a stochastic sequential sampling model that describes choices and response times observed in tasks with a circular decision space (Smith, 2016). The CDDM belongs to the family of evidence accumulation models (EAMs), which share the core assumption that participants sample and accumulate information in favor of the response alternatives over time, until a response criteria is met. In the CDDM, this accumulation process is characterized as a random walk that starts at the origin of a circle representing the decision space, moving towards its circumference. Once the circumference is reached, a choice is made. The choice is then expressed as the angle in radians of the point of intersection and the response time is captured by the amount of steps it took to reach the circumference.

Much like other evidence accumulation models (especially the classical drift-diffusion model), the CDDM is defined by (1) a boundary parameter $\eta$ that serves as a response criterion, (2) a non-decision time parameter $\tau$ that captures the time consumed by secondary processes not related to the decision process (e.g., motor control or information processing), and (3) a doublet of parameters describing the overall speed and direction of the information accumulation process. These last two parameters can be expressed in cartesian coordinates (i.e., the mean displacement observed on the X and Y coordinates per unit of time $\{\mu_x, \mu_y\}$) or polar coordinates (i.e., the average speed and expected direction $\{\delta,\theta\}$).

The following figure illustrates the CDDM in a graphical representation.
...
need_figure <- file.exists(here("output", "sample-figures", "CDDM_fig1.png"))
if(!need_figure){
    ...
    png(here("output", "sample-figures", "CDDM_fig1.png"), width = 1000, height = 1000)
    data_fig1 <- plot.CDDM_Fig1(trials = n, par, return.Data = TRUE)
    dev.off()
}

knitr::include_graphics(here("output", "sample-figures", "CDDM_fig1.png"))
...
# Gold standard: Random-walk emulator

The first sampling algorithm we will consider is the Random Walk Emulator (RWE). This algorithm is based on the random walk process described by the CDDM. We emulate the information accumulation process by iteratively sampling random movements on the X and Y direction using $\mu_x$ and $\mu_y$, until the boundary is reached.
...
This algorithm sets the gold standard to compare the other sampling algorithms against. The random walk emulator provides the most accurate samples, as it is a direct implementation of the model's assumptions. However, it is also the most computationally demanding algorithm, as it requires a large number of iterations to generate a single sample.
...
if(forceRun_RW|forceAll|!file.exists(here("tmp", "X.RW.RData"))){
      start_time <- Sys.time()
      X.RW <- rCDDM_RandomWalk(n,par)
      end_time <- Sys.time()
      RW.time <- round(end_time-start_time,4)
      save(X.RW, file = here("tmp", "X.RW.RData"))
      save(RW.time, file = here("tmp", "RW.time.RData"))
}else{
     load(here("tmp", "X.RW.RData"))    
     load(here("tmp", "RW.time.RData"))
}
```

```{r plot_RWchoices, fig.height=5, echo=FALSE}
...
# CDDM probability functions

## dCDDM(*) - density function

The probability density function for the Circular Drift Diffusion Model (CDDM) is given by:

$$
f(c, t | \nu, \theta, \eta, t_0) = 
\begin{cases}
0 & \text{if } t \leq t_0 \\
\frac{1}{2\pi\eta^2} \exp\left(-\frac{1}{2}\left[\nu^2(t-t_0) - 2\eta\nu\cos(c-\theta)\right]\right) \sum_{i=1}^{\infty} \frac{j_{0,i}}{J_1(j_{0,i})} \exp\left(-\frac{j_{0,i}^2}{2\eta^2}(t-t_0)\right) & \text{if } t > t_0
\end{cases}
$$

where:

- $c$ is the choice angle (in radians)
- $t$ is the response time (in seconds)
- $\nu$ is the drift rate (strength of evidence accumulation)
- $\theta$ is the drift direction (in radians)
- $\eta$ is the boundary separation
- $t_0$ is the non-decision time
- $j_{0,i}$ is the $i$-th zero of the Bessel function $J_0$
- $J_1$ is the Bessel function of the first kind of order 1

In practice, the infinite sum is approximated using a finite number of terms (typically 50).
...
```{r, fig.height=5, echo=FALSE}
x <- dCDDM(X.RW$bivariate.data, drift, theta, tzero, boundary)

plot.CDDM_3D_density(bivariate.data = X.RW$bivariate.data,
                     densities = x,
                     par = par)
```


## pCDDM(*) - cumulate density function

```{r intro_pCDDM}
source(here("code", "cddm", "pCDDM_aux.R"))
source(here("code", "cddm", "pCDDM.R"))

x <- pCDDM(X.RW$bivariate.data, drift, theta, tzero, boundary, show=TRUE)
```

# Algorithm 2: Rejection sampling algorithms

```{r, eval=FALSE}
source("./code/cddm/sim_Rejection.R")
X.Reject_Uniform <- rCDDM_Reject(n,par,type="Uniform",plot=TRUE)
X.Reject_2DNormal <- rCDDM_Reject(n,par,type="2DNormal",plot=TRUE)
X.Reject_exGvonM <- rCDDM_Reject(n,par,type="exGvonM",plot=TRUE)
```

```{r,  echo=FALSE, fig.cap="Fig.2 Visual representation of the Reject sampling algorithm. The blue lines represent the joint density function of response times and choices prescribed by the CDDM. The dots represent the pairs of candidate value generated from the bivariate space of possible RTs and choices, plotted at the height of the random uniform(0, maxDensity) value used as rejection criterion. If the rejection criterion was lower than the density function (i.e., dot falls below density curve), the sample is accepted; if the rejection value surpasses the density (i.e., dot is drawn above), the candidate is rejected. The process of generating, testing and keeping candidates is repeated until the desired sample size is achieved.", warning=FALSE}
# The r-chunk above is actually just for display. This is where the samples are actually
# generated or loaded from previously-saved output files, as indicated in the opening chunk.
if(forceRun_Rej|forceAll|!file.exists("./tmp/X.Reject_Uniform.RData")){
      # Rejection sampling with Uniform proposal distribution
      start_time_RUniform <- Sys.time()
      X.Reject_Uniform <- rCDDM_Reject(n,par,type="Uniform",plot=FALSE)
      end_time_RUniform <- Sys.time()
      Reject_Uniform.time <- round(end_time_RUniform-start_time_RUniform, 4)
      save(X.Reject_Uniform, file = "./tmp/X.Reject_Uniform.RData")
      save(Reject_Uniform.time, file = "./tmp/Reject_Uniform.time.RData")

      # Rejection sampling with 2D Normal proposal distribution
      start_time_R2DNormal <- Sys.time()
      X.Reject_2DNormal <- rCDDM_Reject(n,par,type="2DNormal",plot=FALSE)
      end_time_R2DNormal <- Sys.time()
      Reject_2DNormal.time <- round(end_time_R2DNormal-start_time_R2DNormal, 4)
      save(X.Reject_2DNormal, file = "./tmp/X.Reject_2DNormal.RData")
      save(Reject_2DNormal.time, file = "./tmp/Reject_2DNormal.time.RData")

      # Rejection sampling with ex-Gaussian proposal distribution
      start_time_RexGvonM <- Sys.time()
      X.Reject_exGvonM <- rCDDM_Reject(n,par,type="exGvonM",plot=FALSE)
      end_time_RexGvonM <- Sys.time()
      Reject_exGvonM.time <- round(end_time_RexGvonM-start_time_RexGvonM, 4)
      save(X.Reject_exGvonM, file = "./tmp/X.Reject_exGvonM.RData")
      save(Reject_exGvonM.time, file = "./tmp/Reject_exGvonM.time.RData")
      par(mfrow=c(2,2), mar = c(0, 0, 0, 0), mai = c(0, 0, 0, 0))
      z1 <- rCDDM_Reject(n,par,type="Uniform",plot=TRUE)
      z2 <- rCDDM_Reject(n,par,type="2DNormal",plot=TRUE)
      z3 <- rCDDM_Reject(n,par,type="exGvonM",plot=TRUE)
}else{
     # Load already existing output
     load("./tmp/X.Reject_Uniform.RData")
     load("./tmp/Reject_Uniform.time.RData")
     load("./tmp/X.Reject_2DNormal.RData")
     load("./tmp/Reject_2DNormal.time.RData")
     load("./tmp/X.Reject_exGvonM.RData")
     load("./tmp/Reject_exGvonM.time.RData")
...
```{r, fig.height=5}
par(pty="s", mfrow=c(2,2), mar = c(0, 0, 0, 0), mai = c(0, 0, 0, 0)) 
plot.CDDM_choiceData(X.Reject_Uniform, par, choice.col.RGB = rgb.Reject)
text(0, 0, "Uniform")
text(0, -0.6, "proposal distribution")
plot.CDDM_choiceData(X.Reject_2DNormal, par, choice.col.RGB = rgb.Reject)
text(0, 0, "Bivariate Normal")
text(0, -0.6, "proposal distribution")
plot.CDDM_choiceData(X.Reject_exGvonM, par, choice.col.RGB = rgb.Reject)
text(0, 0, "ex-Gaussian and vonMisses")
text(0, -0.6, "proposal distribution")
```

```{r, fig.height=5}
maxY <- max(density(X.RW$bivariate.data[,2])$y)

par(mfrow=c(2,2), mar = c(0, 0, 0, 0), mai = c(0, 0, 0, 0)) 
myRThistogram(X.Reject_Uniform[,2], color=col.Reject, maxY=maxY)
lines(density(X.RW$bivariate.data[,2]), col = col.RW(1), lwd=3)
text(quantile(X.Reject_Uniform[,2], 0.85), maxY*0.75, "Uniform", col="gray20")
myRThistogram(X.Reject_2DNormal[,2], color=col.Reject, maxY=maxY)
lines(density(X.RW$bivariate.data[,2]), col = col.RW(1), lwd=3)
text(quantile(X.Reject_2DNormal[,2], 0.85), maxY*0.75, "Bivariate Normal", col="gray20")
myRThistogram(X.Reject_exGvonM[,2], color=col.Reject, maxY=maxY)
lines(density(X.RW$bivariate.data[,2]), col = col.RW(1), lwd=3)
text(quantile(X.Reject_exGvonM[,2], 0.85), maxY*0.75, "ex-Gaussian and vonMisses", col="gray20")
```

```{r, fig.height=6}
plot.CDDM_margECDF(X.Reject_Uniform, color=col.Reject(0.3))
plot.CDDM_margECDF(X.Reject_2DNormal, color=col.Reject(0.3))
plot.CDDM_margECDF(X.Reject_exGvonM, color=col.Reject(0.3))
```
...
```{r, eval=FALSE}
source(here("code", "cddm", "sim_Metropolis.R"))
X.Metro <- rCDDM_Metropolis(n,par,plot=TRUE)
```
...
```{r, eval=FALSE}
source(here("code", "cddm", "sim_invCDF.R"))
...
axis(1,seq(min(allRT),max(allRT),round(seq(min(allRT),max(allRT),length.out=10))
