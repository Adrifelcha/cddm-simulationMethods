...
X.invCDF <- sample.invCDF.cddm(n, par, plot=TRUE)
```
...
# Plot the eCDFs obtained so far
par(pty="s", mfrow=c(2,2),mar = c(0, 0, 0, 0)) 
scatterplot3d(X.RW$bivariate.data[,1], X.RW$bivariate.data[,2], RW.tCDF, xlim = x.lim,  
              pch=3, cex.symbols = 0.3, xlab = "", ylab = "", zlab = "")
mtext("Random Walk algorithm", f=2)
mtext("Choices", side=1, line=0.5)
mtext("RTs", side=4, line=-1, adj=0)
scatterplot3d(X.Reject[,1],X.Reject[,2],Reject.tCDF, pch=3, xlim = x.lim,   
              cex.symbols = 0.3, xlab = "", ylab = "", zlab = "")
mtext("Rejection sampling algorithm", f=2)
mtext("Choices", side=1, line=0.5)
mtext("RTs", side=4, line=-1, adj=0)
scatterplot3d(X.Metro[,1],X.Metro[,2],Metro.tCDF, pch=3, xlim = x.lim,   
              cex.symbols = 0.3, xlab = "", ylab = "", zlab = "")
mtext("Metropolis sampling algorithm", f=2)
mtext("Choices", side=1, line=0.5)
mtext("RTs", side=4, line=-1, adj=0)
scatterplot3d(X.invCDF[,1],X.invCDF[,2],invCDF.tCDF, pch=16, xlim = x.lim,  
