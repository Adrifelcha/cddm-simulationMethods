...
compareTime_plot <- function(n.Samples,n.Datasets,
    ...
    axis(2, seq(0,max.CompTime,length.out=10), seq(0,max.CompTime,length.out=10), 
         las=2, line=-1.1)
...
if(forceRun_Rej|!file.exists("./tmp/X.Reject_2.RData")){
      X.Reject <- sample.Reject.cddm(n,par2,max.RT,plot=FALSE)
      save(X.Reject, file = "./tmp/X.Reject_2.RData")
   }else{     load("./tmp/X.Reject_2.RData")          }

if(forceRun_Met|!file.exists("./tmp/X.Metro_2.RData")){
      X.Metro <- sample.Metropolis.cddm(n,par2,max.RT,plot=FALSE)
      save(X.Metro, file = "./tmp/X.Metro_2.RData")
   }else{     load("./tmp/X.Metro_2.RData")          }

if(forceRun_inv|!file.exists("./tmp/X.invCDF_2.RData")){
      X.invCDF <- sample.invCDF.cddm(n,par2,max.RT,plot=FALSE)
      save(X.invCDF, file = "./tmp/X.invCDF_2.RData")
   }else{     load("./tmp/X.invCDF_2.RData")          }
...
RW.tCDF <- pCDDM(X.RW$bivariate.data, par2$drift, par2$theta, par2$tzero, par2$boundary)
Reject.tCDF <- pCDDM(X.Reject, par2$drift, par2$theta, par2$tzero, par2$boundary)
Metro.tCDF <- pCDDM(X.Metro, par2$drift, par2$theta, par2$tzero, par2$boundary)
invCDF.tCDF <- pCDDM(X.invCDF, par2$drift, par2$theta, par2$tzero, par2$boundary)
```
...
hist(X.Metro[,2], main = "", col=col.Metro(0.7))
mtext("Metropolis algorithm", 3, f=2, cex=0.8)
hist(X.invCDF[,2], main = "", col=col.invCDF(0.7))
mtext("inverse CDF algorithm", 3, f=2, cex=0.8)
mtext("Response Times sampled", outer=TRUE, line=-1.5, f=2, cex=2)
```

```{r, echo=FALSE}
# Plot the eCDFs obtained so far
par(pty="s")          
par(mfrow=c(1,2),mar = c(0, 0, 0, 0), mfrow=c(2,2)) 
scatterplot3d(X.RW$bivariate.data[,1], X.RW$bivariate.data[,2],
...
mtext("RTs", side=4, line=-1, adj=0)
scatterplot3d(X.Metro[,1],X.Metro[,2],Metro.eCDF, pch=16,
              cex.symbols = 0.5, color = col.Metro(0.4), 
              xlab = "", ylab = "", zlab = "")
mtext("Metropolis sampling algorithm", f=2)
mtext("Choices", side=1, line=0.5)
mtext("RTs", side=4, line=-1, adj=0)
scatterplot3d(X.invCDF[,1],X.invCDF[,2],invCDF.eCDF, pch=16,
              cex.symbols = 0.5, color = col.invCDF(0.4), 
              xlab = "", ylab = "", zlab = "")
mtext("inverse CDF sampling algorithm", f=2)
mtext("Choices", side=1, line=0.5)
mtext("RTs", side=4, line=-1, adj=0)
mtext("Empirically derived CDFs", outer=TRUE, line=-2, f=2, cex=2)
```
...
mtext("Choices", side=1, line=0.5)
mtext("RTs", side=4, line=-1, adj=0)
mtext("Approximated CDFs", outer=TRUE, line=-2, f=2, cex=2)
```
