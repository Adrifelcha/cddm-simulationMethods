---
title: "A comparison between sampling approaches for the CDDM in R"
...
x <- pCDDM(data_fig1, drift, theta, tzero, boundary, show=TRUE)
```
...
```{r, fig.height=5}
par(mfrow=c(2,2), mar = c(0, 0, 0, 0), mai = c(0, 0, 0, 0)) 
myRThistogram(X.Reject_Uniform[,2], color=col.Reject)
lines(density(X.RW$bivariate.data[,2]), col = col.RW(1), lwd=3)
myRThistogram(X.Reject_2DNormal[,2], color=col.Reject)
lines(density(X.RW$bivariate.data[,2]), col = col.RW(1), lwd=3)
myRThistogram(X.Reject_exGvonM[,2], color=col.Reject)
lines(density(X.RW$bivariate.data[,2]), col = col.RW(1), lwd=3)
```
