...
Each pair of observations is obtained by emulating the random walk process described by the CDDM and recording the point at which the circumference is crossed and the number of steps it took as our choice and response times.
...
hist(X.RW$bivariate.data[,2], main = "Response Times", col = rgb(0.4,0.9,0.3,0.7))
```
...
if(forceRun|!file.exists("./tmp/X.MCMC.RData")){
     ...
     sample.MCMC.cddm(n,par,max.RT,plot=TRUE)
}
```
...
```{r, fig.height=5}
par(pty="m", mar = c(3, 3, 3, 0)) 
hist(X.MCMC[,2], main = "Response Times", col=rgb(0.15,.29,.80,0.7))
```
...
range(times.RW, na.rm = TRUE)
range(times.MCMC, na.rm = TRUE)
```
