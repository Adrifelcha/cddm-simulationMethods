...
hist(X.RW$bivariate.data[,2], main = "Random Walk", col = col.RW(0.7))
par(pty="m", mar = c(3, 3, 3, 0)) 
hist(X.Reject[,2], main = "Rejection algorithm", col=col.Reject(0.7))
mtext("Response Times sampled", outer=TRUE, line=-1)
...
times.Reject <- matrix(NA, nrow=M, ncol=max(N))
for(m in 1:M){
    for(n in N){
      start_time <- Sys.time()
      sample.RW.cddm(n,par)
      ...
      sample.Reject.cddm(n,par,max.RT,plot=FALSE)
      end_time <- Sys.time()
      times.Reject[m,n] <- round(end_time-start_time, 4)
    }
}
```
...
x.Reject <- (rep(N, each=M)-0.15)+(rnorm(max(N)*M,0,0.015))
t.RW <- c(times.RW)
t.Reject <- c(times.Reject)
